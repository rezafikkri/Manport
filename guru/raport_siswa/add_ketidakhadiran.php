...
<div class="col-6 offset-left-3 offset-right-3">
	<div class="home default">
		<h1 class="judul marginBottom20px">Tambah ketidakhadiran <span><?= $dbS->get_one_siswa_detail($siswa_detail_id, "masih_sekolah", 'sd.nama_siswa', $_SESSION['RAPORT']['kelas_id'])['nama_siswa']??''; ?></span></h1>
		<form id="form" action="raport_siswa/proses.php?action=add_edit_ketidakhadiran" method="post">
			...
			<?= $db->pesan_add_ketidakhadiran(); ?>
			<label class="label">Sakit</label>
			<?= $errors['sakit']??''; ?>
			<input type="text" name="sakit" placeholder="..." value="<?= $old['sakit']??''; ?>">
			<label class="label">Izin</label>
			<?= $errors['izin']??''; ?>
			<input type="text" name="izin" placeholder="..." value="<?= $old['izin']??''; ?>">
			<label class="label">Tanpa keterangan</label>
			<?= $errors['tanpa_keterangan']??''; ?>
			<input type="text" name="tanpa_keterangan" placeholder="..." value="<?= $old['tanpa_keterangan']??''; ?>">
			<label class="label">Bolos</label>
			<?= $errors['bolos']??''; ?>
			<input type="text" name="bolos" placeholder="..." value="<?= $old['bolos']??''; ?>">
