...
<div class="col-6 offset-left-3 offset-right-3">
	<div class="home default">
		<h1 class="judul marginBottom20px">Edit catatan wali kelas <span><?= $dbS->get_one_siswa_detail($siswa_detail_id, "masih_sekolah", 'sd.nama_siswa', $_SESSION['RAPORT']['kelas_id'])['nama_siswa']??''; ?></span></h1>
		<form id="form" action="raport_siswa/proses.php?action=add_edit_catatan_wali_kelas" method="post">
			...
			<?= $db->pesan_edit_catatan_wali_kelas(); ?>
			<label class="label">Catatan wali kelas</label>
			<?= $errors['catatan']??''; ?>
			<textarea spellcheck="false" placeholder="..." rows="5" name="catatan"><?= $db->tampil_catatan_wali_kelas($siswa_detail_id, $_SESSION['RAPORT']['tahun_ajaran_id'], $_SESSION['RAPORT']['semester_id'])['catatan']??''; ?></textarea>
